...
class BCQ(object):
	...
	def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, logger=dict()):

		for it in range(iterations):

			# Sample replay buffer / batch
			state_np, next_state_np, action, _, reward, done = replay_buffer.sample(batch_size)
			...
			if (it + 1) % 10 == 0:
				logger.store(QLoss=critic_loss.cpu().item(),
							 Q1Vals=current_Q1.detach().cpu().numpy(),Q2Vals=current_Q2.detach().cpu().numpy(),
							 ActLoss=actor_loss.cpu().item())
